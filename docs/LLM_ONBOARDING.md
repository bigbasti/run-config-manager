...
src/
  ...
  services/                         # runtime orchestration (no UI)
    ...
    buildActions.ts                 # right-click action resolution (build, npm:, pip:)
  ...
  utils/
    ...
    uuid.ts
media/
  icons/                            # brand SVGs (npm/node/angular/vite/python/django/...)
...
- **`prepareLaunch?(cfg, folder, ctx)`** — pre-spawn hook. `ctx` shape:
  ...
  ```
  Returns `{ env?, cwd?, extraArgs?, cfg? }`. Used by Tomcat (CATALINA_BASE scaffold), Spring Boot (color flags + JDWP init script + monitor init script), Java (JDWP init script + monitor init script), Quarkus (`-Djvm.args=...`), npm/python (Node selection / pip pre-flight).
...
- **Launch** (npm, spring-boot/java-main, java/java-main, python): `vscode.debug.startDebugging(folder, getDebugConfig(cfg))`.
...
Tree emits `runConfig.edit` on item click → single-click opens the editor. Tooltip uses `buildCommandPreview`. **`contextValue` shape**: `${baseContextValue}${toolSuffix}${groupSuffix}${monitoredSuffix}` — e.g. `configIdle:maven:grouped:monitored`. Menu when-clauses must tolerate the `:monitored` tail (see JVM monitoring section).
...
**Test count**: 877+ across 83 files. Includes `MonitoringService` lifecycle (8 tests), `buildMonitorJvmArgs`, `freePort`, `parseClassHistogram`, plus per-adapter tests. Start with `ExecutionService.test.ts` or `RunConfigService.test.ts` as templates. Keep fixtures inline.
...
- **Auto Create + Stop All** — title-bar buttons. Auto-create priority: `spring-boot > quarkus > tomcat > java > python > npm`.
...
3. Reproduce the issue in a test if possible — Jest is fast (~3s for 877 tests).
